...
const fileNameRegex =
  /^([0][1-9]|[12][0-9]|3[01])-([0][1-9]|[1][0-2])+-([0-9]{4})+_[FA]N\.xlsx$/;
...
const getExams = async (req, res) => {
  ...
  try {
    ...
    const schedules = await Schedule.find({
      ...
      .select("sem branch slot subcode")
      .lean();
...
const createAllocation = async (req, res) => {
  ...
  try {
    ...
    if (schedule) {
      ...
      res
        .status(404)
        .json({ error: "No schedule found for the given date and time" });
    }
  } catch (error) {
    return res.status(500).json({ message: error.message });
  }
...
const getAllocation = async (req, res) => {
  ...
  try {
    ...
    const allocations = await Allocations.find({
      ...
      .select("rooms seats")
      .lean();
    const rooms = allocations?.flatMap((allocation) =>
      allocation.rooms.map((item) => item.room_no)
    );
