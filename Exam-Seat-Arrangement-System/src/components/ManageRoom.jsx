...
export default function ManageRoom() {
  ...
  const list =
    sortTerm && sortTerm !== "asc" && sortTerm !== "desc"
      ? filteredRows
      : sortedRows;
  ...
  const handleRoom = (e) => {
    ...
    const newRoom = {
      ...
      floor_no: Number(
        floorNoRef.current.options[floorNoRef.current.selectedIndex].value
      ),
      block: blockRef.current.options[blockRef.current.selectedIndex].value,
  ...
  const handleDelete = (room) => {
    ...
    const deleteRoom = async () => {
      try {
        ...
        isMounted &&
          setRows((prev) => prev.filter((item) => item.room_no !== room));
        setLoading(false);
      } catch (error) {
        console.log(error);
      }
  ...
  return (
    <div className="bg-background flex flex-col flex-grow md:w-5/6">
      <div className="px-8 pt-4 mt-6">
        ...
        <form
          ref={formRef}
          className="flex hw:flex-col flex-row justify-between"
          onSubmit={handleRoom}
        >
          <Input
            input_id="room-no"
          ...
          />
          <button
            className="bg-blue-500 hover:bg-blue-400 text-white font-Outfit-Bold py-1 px-2 my-7 mx-2 h-10 w-[5rem] rounded-[20px]"
            type="submit"
          >
            ADD
          </button>
      ...
      <div className="px-8 py-4">
        ...
        <div
          className={`flex flex-wrap items-center bg-gray-100 p-4 rounded-md font-Outfit-Regular`}
        >
          {/* Search Bar */}
          ...
          <div className="flex-grow flex flex-row">
            ...
            <div className="flex-grow flex flex-row items-center ">
              ...
              <select
                className="w-full p-[10.4px] m-1 rounded-md focus:outline-none focus:ring-2 focus:ring-green-login text-gray-600"
                defaultValue=""
            ...
            <div className="flex-grow flex flex-row items-center">
              ...
              <select
                className="w-full p-[10.4px] m-1 mr-2 rounded-md focus:outline-none focus:ring-2 focus:ring-green-login text-gray-600"
                defaultValue="nil"
                onChange={(e) => setFilterTerm(e.target.value)}
              >
                <option value="nil">-</option>
                <option value="Ramanujan">A</option>
                <option value="M-George">B</option>
          ...
          <table className="table-auto w-full">
            <thead className="sticky top-0">
              <tr className="bg-grey-all font-Outfit-Bold">
                <th className="text-center px-4 py-2 rounded-tl-2xl rounded-bl-2xl">
                  <span className="whitespace-nowrap">Room No</span>
                </th>
                <th className="text-center px-4 py-2">
                  <span className="whitespace-nowrap">Floor No</span>
                </th>
                <th className="text-center px-4 py-2">
                  <span className="whitespace-nowrap">Block</span>
                </th>
                <th className="text-center px-4 py-2">
                  <span className="whitespace-nowrap">Available Seats</span>
                </th>
      ...
      <div className="px-8 py-4 mt-4">
        <div className="flex flex-row justify-between items-center">
          <div>
            <p className="font-Outfit-Regular">
              Total Seats Available: {totalCapacity}
            </p>
          </div>
          <div className="flex flex-row gap-10">
            <button
              className="bg-gray-500 hover:bg-gray-400 text-white font-Outfit-Bold h-10 w-[10rem] rounded-[20px]"
